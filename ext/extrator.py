...
def main(mali_data_path, ben_data_path, metric_data_path, save_path):
    # generate labelled malicious data
    csvparser = CsvParser(ben_data_path)
    ben_csv_df = csvparser.fea_ext()
    ...
    total_df = pd.concat([ben_csv_df, mal_df_labels], ignore_index=True)
    ...
    total_df.to_pickle(save_path.joinpath("label_data.parquet"))
...
if __name__ == "__main__":
    ...
    ben_data_path = Path.cwd().parent.joinpath("data","package-analysis-labels.parquet")
    metric_data_path = Path.cwd().parent.joinpath("data","stas", 'data_metrics.csv')
    save_path = Path.cwd().parent.joinpath("data")
    main(mali_data_path, ben_data_path, metric_data_path, save_path)